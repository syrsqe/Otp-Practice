...
Annoncement = mongoose.model("Annoncement", announcementSchema);
    ...
    Table.findOne({
      ...
      if (err) {
        ...
        for (let i = 1; i < tableRows.length; i++) {
          ...
          User.findOne({
            employeeID: tableRows[i].rowContent.slice(0, 4)
            ...
            if (i >= (tableRows.length - 1)) {
              res.render("home", {
                workingThisWeek: workingThisWeek,
    ...
    if (err) {
      ...
      Table.findOneAndUpdate({
        ...
        $set: {
          ...
          htmlCode: currentTable.htmlCode
        }
      }, function(err, foundTable) {
...
app.post("/announcements", isLoggedIn, function(req, res) {
  ...
  newAnnouncement = new Annoncement({
    text: req.body.newAnnouncement,
    date: todaysDate
  });
